...
*
*           Jacobian of the 'Observation Model' (or) the mapping from system state to sensor state
*           shoots up with nan (or) inf values due to division by zero and hence does not produce
*           usable data leading to an estimated state full of 'nan's. The functions and variables
*           regarding that have been commented in the code. Though, I don't think it is possible to
*           manipulate it in anyway, someone interested in doing it may look into those functions.
*           The jacobian matrices were generated from Matlab, and the code is available in the file,
*           'jacobian_cpp.m'.
*
...
{
/**
Car dynamics has been chosen to be Kinematic Bicycle. Currently, only this
can be used, as the EKF code explicitely takes up the beta value. If the
Dubins Car (or) Simple Car model is used, the theta_dot value will automatically
be used as the beta value, whihc could change the parameters adversely.
*/
    kinematic_bicycle c;

/// Simulation parameters, time = 20 seconds, dt = 0.01 seconds, acc = 0.3 m/s^2, phi = 10 degrees/sec
    float time = 20.0;
    float del_t = 0.01;
    float acc = 0.3;
    float phi = d2r(10.0);
