...
*    the elements cannot be accessed
*
...
/*
matrix<double> ekf::get_sensor_projected_state(matrix <double>& predicted_state, float del_t)
{
    double theta = X(2,0);
    double v = X(3,0);
    double beta = X(4,0);
    ...
    hi(1,0) = 0; hi(1,1) = 0.5*std::pow(del_t, 2); hi(1,2) = 0; hi(1,3) = 0; hi(1,4) = v*sin(theta+beta)*del_t;
    ...
    matrix<double> projected_state = matrix<double> (prod(hi, predicted_state));
...
//    matrix<double> projected_state_vector = get_sensor_projected_state(predicted_x, del_t);
//    matrix<double> diff = measurement_vector - projected_state_vector;
//    matrix <double> H = get_H(del_t);
    matrix<double> diff = measurement_vector - predicted_x;
    identity_matrix <double> H(5,5);
